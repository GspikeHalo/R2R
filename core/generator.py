...
class Conformer(nn.Module):
    ...
    def forward(self, x, s, masks=None):
        ...
        # for idx, (up_block, tok_up) in enumerate(zip(self.up_blocks, self.tok_ups)):
        #     # 从末尾开始取：第 idx 层用 x_ts[-1-idx]
        #     x_t_stage = x_ts[-1 - idx]
        #     B, C, H, W = x.shape
        #     # 计算 token→map 的原始小 spatial
        #     H0, W0 = H // tok_up.up_stride, W // tok_up.up_stride
        #     tok_map = tok_up(x_t_stage, H0, W0)
        #
        #     # 把这一级的 token map 注入
        #     x = up_block(x, s, tok_map=tok_map)
