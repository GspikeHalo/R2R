...
"""
"""
经处理后，得到训练集，训练集分未配对的unpaired/raw-rggb*196（仍在同一场景下，可以看成L相同），以及配对的paired*22，也即paired/camera/anchor-raw-rggb（map矩阵由RAW-to-RAW那篇论文算，二次项映射）
测试集为同样方法得出的paired/camera/raw-rggb*115
"""
...
# matrix是一个16*4的矩阵，映射指的是从相机1的RAW域映射至相机2的RAW域，映射方式为二次项组合
...
BASE_DIR = '/media/Data_2/RAW2RAW/'
RESULT_DIR = '/media/Data_2/R2RResult/'
...
for _pair in pair_data:
  for _cam in cameras:
    ...
    # write_raw_rggb_path = output_dir + 'raw-rggb/'
    # write_raw_vis_path = output_dir + 'vis/'
    write_raw_rggb_path = os.path.join(RESULT_DIR, _pair, _cam, 'raw-rggb/')
    write_raw_vis_path = os.path.join(RESULT_DIR, _pair, _cam, 'vis/')

    if _pair == 'paired/':
      # write_anchor_raw_rggb_path = output_dir + 'anchor-raw-rggb/'
      # write_anchor_raw_vis_path = output_dir + 'anchor-vis/'
      write_anchor_raw_rggb_path = os.path.join(RESULT_DIR, _pair, _cam, 'anchor-raw-rggb/')
      write_anchor_raw_vis_path = os.path.join(RESULT_DIR, _pair, _cam, 'anchor-vis/')
      check_dir(write_anchor_raw_rggb_path)
      check_dir(write_anchor_raw_vis_path)

    if _pair == 'paired/':
      mapping_path = path_to_raw_data + 'mapping/'
      write_raw_rggb_path_mapping = os.path.join(RESULT_DIR, _pair, list(
        set(cameras) - {_cam})[0], 'raw-rggb/')
      write_raw_vis_path_mapping = os.path.join(RESULT_DIR, _pair, list(
        set(cameras) - {_cam})[0], 'vis/')
      write_anchor_raw_rggb_path_mapping = os.path.join(RESULT_DIR, _pair, list(
        set(cameras) - {_cam})[0], 'anchor-raw-rggb/')
      write_anchor_raw_vis_path_mapping = os.path.join(RESULT_DIR, _pair, list(
    ...
    for raw_img_path in all_raw_img_paths:
      ...
      # 处理可能存在的超出white level和black level的像素
